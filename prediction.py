...
class Net(nn.Module):
    ...
    def forward(self, _input, Xavg, Xstd):
        x = _input
        frame_pred = self.head(x.permute(0,3,2,1).contiguous()).permute(0,2,3,1)
...
def load_te_mp3(name,avg, std):
    ...
    #x = np.concatenate((x[:ydata.shape[0],:,:,:], ydata),3)
    return x
...
def main(argv):
    ...
    save_dic = torch.load('./data/medley') 
    ...
    all_pred = np.zeros((Xte.shape[0],10,int(Xte.shape[1]/1)))
    ...
    for idx,_input in enumerate(loader):
        ...
        all_pred[ds: ds + len(data)] = F.sigmoid(pred.view(pred.size()[0],pred.size()[2],pred.size()[3])).permute(0,2,1).data.cpu().numpy()
        ds += len(data)
...
if __name__ == "__main__":
   #main(sys.argv[1:])
    #main('make_you_feel_my_love.mp3')
    for n in os.listdir('mp3/'):
        main(n)
