...
class Model(object):
    ...
    def get_data(self,rec_id,s,pitch_shift,scaling_factor):
        ...
        if self.mmap:
            if self.isTest:
                x = np.frombuffer(rec_id[s*sz_float:int(s+scaling_factor*self.window)*sz_float], dtype=np.float32).copy()
            else:
    ...
    def sample_records(self, rec_id, count, fixed_stride=-1, pitch_shift=0):
        mse = 0.
        mse_breakdown = dict()
        avp_breakdown = dict()
        if self.isTest:
            count = int((len(rec_id)/4-16384)/512)-10
        else:
            count = int(((len(self.data[rec_id][0])/4)-16384)/512)-10
        
        offset = 0# 44100
        Yhat = np.empty((count,self.m))
        Yall = np.empty((count,self.m))
        sf = 2.**(pitch_shift/12.)
        ...
        if self.isTest:
            stride = (len(rec_id)/4-offset-int(sf*self.window))/count if fixed_stride==-1 else fixed_stride
        else:
            stride = (self.data[rec_id][1]-offset-int(sf*self.window))/count if fixed_stride==-1 else fixed_stride
        
            # stride 512
        for j in range(count):
                # sf 1
            #print str(j)+'/'+str(count)
            X[j,0,:,0],Y[j] = self.get_data(rec_id,offset+j*stride,pitch_shift=int(round(pitch_shift)),scaling_factor=sf)
        ...
        return None, Yhat, Yall, None, None
